...
import { ArrowLeft, Clock, CheckCircle, XCircle, RotateCcw, Eye } from "lucide-react"
...
export default function ModuleQuizPage() {
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      try {
        ...
        const moduleIndex = Number.parseInt(moduleId) - 1
        if (data.quizzes && data.quizzes[moduleIndex]) {
          setQuestions(data.quizzes[moduleIndex].questions)
        }
        setLoading(false)
      } catch (error) {
  ...
  if (questions.length === 0) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 flex items-center justify-center">
        <div className="text-center">
          <p className="text-slate-600 dark:text-gray-300">No questions found for this module.</p>
          <Link href="/modules">
        ...
        <nav className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50 dark:bg-gray-900/80 dark:border-gray-700">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-16">
              ...
              <span className="text-lg font-semibold text-slate-800 dark:text-white">Module {moduleId} Results</span>
            </div>
          </div>
        </nav>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <Card className="border-0 shadow-xl dark:bg-card dark:text-card-foreground dark:shadow-lg">
            <CardHeader className="text-center pb-6">
              ...
              <CardTitle className="text-3xl mb-2">{passed ? "Congratulations!" : "Keep Practicing!"}</CardTitle>
              <p className={`text-xl ${passed ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
                {passed ? "You have passed!" : "You need more practice."}
              </p>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid md:grid-cols-3 gap-4 text-center">
                <div className="bg-slate-50 rounded-lg p-4 dark:bg-secondary">
                  ...
                  <div className="text-slate-600 dark:text-gray-300">Score</div>
                </div>
                <div className="bg-slate-50 rounded-lg p-4 dark:bg-secondary">
                  <div className="text-2xl font-bold text-slate-800 dark:text-white">
                    {Math.round((score / questions.length) * 100)}%
                  </div>
                  <div className="text-slate-600 dark:text-gray-300">Percentage</div>
                </div>
                <div className="bg-slate-50 rounded-lg p-4 dark:bg-secondary">
                  ...
                  <div className="flex items-start space-x-3">
                    <div
                      className={`w-6 h-6 ${isMultipleChoice ? "rounded-md" : "rounded-full"} border-2 flex items-center justify-center mt-0.5 ${
                        isSelected ? "border-white bg-white" : "border-slate-300 dark:border-gray-500"
                      }`}
                    >
                      {isSelected && (
                        <div
                          className={`w-3 h-3 ${isMultipleChoice ? "rounded-sm" : "rounded-full"} bg-blue-500`}
                        ></div>
                      )}
                    </div>
